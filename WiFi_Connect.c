...
/*Global variable to store the BLE configuration network status*/
...
uint8_t wifi_connect_status = CONNECT_FAILED;   //wifi_connect_status 0: connect success, 1: connect failed, 2: retry connect
uint8_t BLE_CONFIG_NET = DISABLE;
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < DEFAULT_ESP_MAXIMUM_RETRY) {
            esp_wifi_connect();
            s_retry_num++;
            wifi_connect_status = RETRY_CONNECT;
        ...
        s_retry_num = 0;
        wifi_connect_status = CONNECT_SUCCESS;
